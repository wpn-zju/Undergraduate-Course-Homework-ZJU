...
static int myext2_dax_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	...
	ret = __dax_fault(vma, vmf, myext2_get_block, NULL);
						...
						pmd_t *pmd, unsigned int flags)
{
	...
	ret = __dax_pmd_fault(vma, addr, pmd, flags, myext2_get_block, NULL);
...
{		
	int i;
	ssize_t ret = my_read(filp, buf, len, ppos);
	for(i = 0;i < len;i++)
	{
		buf[i] = (buf[i] -25 + 128) % 128;
	}
	printk("haha encrypt %u\n", len);
	return ret;
}
